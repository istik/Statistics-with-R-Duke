...
sfo_feb_flights %>%
  summarise(mean = mean(arr_delay), max=max(arr_delay), median(arr_delay),n = n())

#<li> No flight is delayed more than 2 hours. </li>  false
...
sfo_feb_flights %>%
        group_by(carrier) %>%
        summarise(median(arr_delay),IQR(arr_delay))
...
nycflights %>%
        group_by(origin) %>%
        summarise(ontime_rate = sum(dep_type=="on time")/n()) %>%
...
#    dep_type sum(arr_type == "on time")/n()
#    (chr)                          (dbl)
#1  delayed                      0.1833639
#2  on time                      0.7762241     
```